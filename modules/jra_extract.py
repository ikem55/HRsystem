...
class JRAExtract(BaseExtract):
    ...
    def get_raceuma_table_base(self):
        ...
        if self.mock_flag:
            ...
            if len(self.raceuma_df.index) == 0:
                ...
                self.raceuma_df = pd.merge(raceuma_df, skb_df, on=["RACE_KEY", "UMABAN", "target_date", "KYOSO_RESULT_KEY"])
    ...
    def get_bac_df(self, filename):
        names = ['RACE_KEY', 'KAISAI_KEY', 'NENGAPPI', '発走時間', '距離', '芝ダ障害コード', '右左', '内外', '種別', '条件', '記号', '重量', 'グレード', 'レース名', '回数', '頭数',
                 'コース', '開催区分', 'レース名短縮', 'レース名９文字', 'データ区分', '１着賞金', '２着賞金', '３着賞金', '４着賞金', '５着賞金', '１着算入賞金', '２着算入賞金',
    ...
    def get_cyb_df(self, filename):
        names = ['RACE_KEY', 'UMABAN', '調教タイプ', '調教コース種別', '調教コース坂', '調教コースW', '調教コースダ', '調教コース芝', '調教コースプール', '調教コース障害',
                 '調教コースポリ', '調教距離', '調教重点', '追切指数', '仕上指数', '調教量評価', '仕上指数変化', '調教コメント', 'コメント年月日', '調教評価', 'target_date']
        if filename[-3:] == "pkl":
    ...
    def get_kab_df(self, filename):
        names = ['KAISAI_KEY', 'NENGAPPI', '開催区分', '曜日', '場名', '天候コード', '芝馬場状態コード', '芝馬場状態内', '芝馬場状態中', '芝馬場状態外', '芝馬場差', '直線馬場差最内',
                 '直線馬場差内', '直線馬場差中',  '直線馬場差外', '直線馬場差大外', 'ダ馬場状態コード', 'ダ馬場状態内', 'ダ馬場状態中', 'ダ馬場状態外', 'ダ馬場差', 'データ区分', '連続何日目',
    ...
    def get_kka_df(self, filename):
        names = ['RACE_KEY', 'UMABAN', 'ＪＲＡ成績１着', 'ＪＲＡ成績２着', 'ＪＲＡ成績３着', 'ＪＲＡ成績着外', '交流成績１着', '交流成績２着', '交流成績３着', '交流成績着外', '他成績１着', '他成績２着', '他成績３着', '他成績着外',
                 '芝ダ障害別成績１着', '芝ダ障害別成績２着', '芝ダ障害別成績３着', '芝ダ障害別成績着外',
                 '芝ダ障害別距離成績１着', '芝ダ障害別距離成績２着', '芝ダ障害別距離成績３着', '芝ダ障害別距離成績着外', 'トラック距離成績１着', 'トラック距離成績２着', 'トラック距離成績３着', 'トラック距離成績着外',
                 'ローテ成績１着', 'ローテ成績２着', 'ローテ成績３着', 'ローテ成績着外', '回り成績１着', '回り成績２着', '回り成績３着', '回り成績着外',
                 '騎手成績１着', '騎手成績２着', '騎手成績３着', '騎手成績着外', '良成績１着', '良成績２着', '良成績３着', '良成績着外', '稍成績１着', '稍成績２着', '稍成績３着', '稍成績着外', '重成績１着',
                 '重成績２着', '重成績３着', '重成績着外', 'Ｓペース成績１着', 'Ｓペース成績２着', 'Ｓペース成績３着', 'Ｓペース成績着外',
                 'Ｍペース成績１着', 'Ｍペース成績２着', 'Ｍペース成績３着', 'Ｍペース成績着外', 'Ｈペース成績１着', 'Ｈペース成績２着', 'Ｈペース成績３着', 'Ｈペース成績着外', '季節成績１着', '季節成績２着',
                 '季節成績３着', '季節成績着外', '枠成績１着', '枠成績２着', '枠成績３着', '枠成績着外', '騎手距離成績１着', '騎手距離成績２着', '騎手距離成績３着', '騎手距離成績着外',
                 '騎手トラック距離成績１着', '騎手トラック距離成績２着', '騎手トラック距離成績３着', '騎手トラック距離成績着外', '騎手調教師別成績１着', '騎手調教師別成績２着', '騎手調教師別成績３着',
                 ...
                 '調教師馬主別成績着外', '父馬産駒芝連対率', '父馬産駒ダ連対率', '父馬産駒連対平均距離', '母父馬産駒芝連対率', '母父馬産駒ダ連対率', '母父馬産駒連対平均距離', 'target_date']
        if filename[-3:] == "pkl":
    ...
    def get_kyi_df(self, filename):
        names = ['RACE_KEY', 'UMABAN', 'NENGAPPI', '血統登録番号', 'IDM', '騎手指数', '情報指数', '総合指数', '脚質', '距離適性', '上昇度', 'ローテーション', '基準オッズ', '基準人気順位', '基準複勝オッズ', '基準複勝人気順位', '特定情報◎', '特定情報○', '特定情報▲', '特定情報△', '特定情報×',
                      '総合情報◎', '総合情報○', '総合情報▲', '総合情報△', '総合情報×', '人気指数', '調教指数', '厩舎指数', '調教矢印コード', '厩舎評価コード', '騎手期待連対率', '激走指数', '蹄コード', '重適正コード', 'クラスコード', 'ブリンカー', '騎手名', '負担重量', '見習い区分', '調教師名',
                      '調教師所属', 'ZENSO1_KYOSO_RESULT', 'ZENSO2_KYOSO_RESULT', 'ZENSO3_KYOSO_RESULT', 'ZENSO4_KYOSO_RESULT', 'ZENSO5_KYOSO_RESULT', 'ZENSO1_RACE_KEY', 'ZENSO2_RACE_KEY', 'ZENSO3_RACE_KEY', 'ZENSO4_RACE_KEY', 'ZENSO5_RACE_KEY', '枠番', '総合印', 'ＩＤＭ印', '情報印', '騎手印', '厩舎印', '調教印', '激走印', '芝適性コード',
                      'ダ適性コード', '騎手コード', '調教師コード', '獲得賞金', '収得賞金', '条件クラス', 'テン指数', 'ペース指数', '上がり指数', '位置指数', 'ペース予想', '道中順位', '道中差', '道中内外', '後３Ｆ順位', '後３Ｆ差', '後３Ｆ内外', 'ゴール順位', 'ゴール差', 'ゴール内外',
                      '展開記号', '距離適性２', '枠確定馬体重', '枠確定馬体重増減', '取消フラグ', '性別コード', '馬主名', '馬主会コード', '馬記号コード', '激走順位', 'LS指数順位', 'テン指数順位', 'ペース指数順位', '上がり指数順位', '位置指数順位', '騎手期待単勝率', '騎手期待３着内率', '輸送区分', '走法',
                      '体型０１', '体型０２', '体型０３', '体型０４', '体型０５', '体型０６', '体型０７', '体型０８', '体型０９', '体型１０', '体型１１', '体型１２', '体型１３', '体型１４', '体型１５', '体型１６', '体型１７', '体型１８',
                      '体型総合１', '体型総合２', '体型総合３', '馬特記１', '馬特記２', '馬特記３', '馬スタート指数', '馬出遅率', '参考前走', '参考前走騎手コード', '万券指数', '万券印', '降級フラグ', '激走タイプ', '休養理由分類コード', '芝ダ障害フラグ', '距離フラグ', 'クラスフラグ', '転厩フラグ', '去勢フラグ', '乗替フラグ', '入厩何走目', '入厩年月日', '入厩何日前', '放牧先',
            ...
            if os.path.exists(self.jrdb_folder_path + 'KYI/' + filename):
                with open(self.jrdb_folder_path + 'KYI/' + filename, 'r', encoding="ms932") as fh:
                    ...
                    for line in fh:
                        ...
                        df = df.append(sr, ignore_index=True)
                df = df.astype({'ローテーション': 'int8', '基準人気順位': 'int8', '基準複勝人気順位': 'int8', '特定情報◎': 'int8', '特定情報○': 'int8',
                                '特定情報▲': 'int8',
    ...
    def get_sed_df(self, filename, type):
        names = ['RACE_KEY', 'UMABAN', '血統登録番号', 'NENGAPPI', 'KYOSO_RESULT_KEY', '馬名', '距離', '芝ダ障害コード', '右左', '内外', '馬場状態', '種別', '条件', '記号', '重量', 'グレード', 'レース名', '頭数', 'レース名略称',
                 '着順', '異常区分', 'タイム', '斤量', '騎手名', '調教師名', '確定単勝オッズ', '確定単勝人気順位', 'ＩＤＭ結果', '素点', '馬場差', 'ペース', '出遅', '位置取', '不利', '前不利', '中不利', '後不利',
                 'レース', 'コース取り', '上昇度コード', 'クラスコード', '馬体コード', '気配コード',
                      'レースペース', '馬ペース', 'テン指数結果', '上がり指数結果', 'ペース指数結果', 'レースＰ指数結果', '1(2)着馬名', '1(2)着タイム差', '前３Ｆタイム', '後３Ｆタイム', '確定複勝オッズ下', '10時単勝オッズ',
                 '10時複勝オッズ', 'コーナー順位１', 'コーナー順位２', 'コーナー順位３', 'コーナー順位４', '前３Ｆ先頭差', '後３Ｆ先頭差', '騎手コード', '調教師コード', '馬体重', '馬体重増減', '天候コード', 'コース', 'レース脚質',
        ...
        if filename[-3:] == "pkl":
            ...
            if os.path.exists(self.jrdb_folder_path + type + '/' + filename):
                with open(self.jrdb_folder_path + type + '/' + filename, 'r', encoding="ms932") as fh:
                    ...
                    for line in fh:
                        ...
                        sr = pd.Series([
                            ...
                            # DEOKURE
                            # ICHIDORI
                            , self.int_0(new_line[180:182]), self.int_0(new_line[182:185]), self.int_0(new_line[185:188]), self.int_0(new_line[188:191]), self.int_0(new_line[191:194]), self.int_0(new_line[194:197]), self.int_0(new_line[197:200]), self.int_0(new_line[200:203])  # FURI
                ...
                if type == "SED":
                    ...
                    df = pd.merge(df, all_sed_df[["RACE_KEY", "UMABAN", "テン指数結果順位", "上がり指数結果順位", "ペース指数結果順位"]], on =["RACE_KEY", "UMABAN"], how="left").fillna(0)
    ...
    def get_srb_df(self, filename):
        ...
        if filename[-3:] == "pkl":
            ...
            if os.path.exists(self.jrdb_folder_path + 'SRB/' + filename):
                ...
                df = pd.merge(df, kab_df[["KAISAI_KEY", "連続何日目", "芝種類", "草丈", "転圧", "凍結防止剤", "中間降水量", "ダ馬場状態コード", "芝馬場状態コード"]], on="KAISAI_KEY")
                ...
                target_df.loc[:, "追走力"] = target_df.apply(lambda x: x["コーナー順位２"] - x["コーナー順位４"] if x["コーナー順位２"] != 0 else x["コーナー順位３"] - x["コーナー順位４"], axis=1)
                ...
                target_df.loc[:, "後傾指数"] = target_df.apply(lambda x: x["１ハロン平均"] * 3 / x["後３Ｆタイム"] if x["後３Ｆタイム"] != 0 else 1, axis=1)
                ...
                gp_raceuma_df = pd.merge(gp_mean_raceuma_result_df, gp_std_raceuma_result_df, on="RACE_KEY")[[
                    'RACE_KEY', '１ハロン平均_mean', 'ＩＤＭ結果_mean', 'テン指数結果_mean', '上がり指数結果_mean', 'ペース指数結果_mean', '前３Ｆタイム_mean',
                    '後３Ｆタイム_mean', 'コーナー順位１_mean', 'コーナー順位２_mean', 'コーナー順位３_mean', 'コーナー順位４_mean', '前３Ｆ先頭差_mean', '後３Ｆ先頭差_mean',
                ...
                zero_group_df.loc[:, "TRACK_BIAS_ZENGO"] = zero_group_df["レース脚質"] + zero_group_df["コーナー順位３"] + zero_group_df["コーナー順位４"]
                zero_group_df.loc[:, "TRACK_BIAS_UCHISOTO"] = zero_group_df["馬番"] + zero_group_df["４角コース取り"]
                df = pd.merge(df, zero_group_df[["RACE_KEY", "TRACK_BIAS_ZENGO", "TRACK_BIAS_UCHISOTO"]], on="RACE_KEY", how="left")
    ...
    def _calc_raptype(self, arr):
        ...
        if len(rap_list) >= 5:
            last_rap = rap_list[-5:]
            rap_diff = [last_rap[3] - last_rap[4], last_rap[2] - last_rap[3], last_rap[1] - last_rap[2], last_rap[0] - last_rap[1]]
    ...
    def get_skb_df(self, filename, type):
        names = ['RACE_KEY', 'UMABAN', '血統登録番号', 'NENGAPPI', 'KYOSO_RESULT_KEY', '特記コード１', '特記コード２', '特記コード３', '特記コード４', '特記コード５', '特記コード６', '馬具コード１', '馬具コード２', '馬具コード３', '馬具コード４',
                 '馬具コード５', '馬具コード６', '馬具コード７', '馬具コード８', '総合１', '総合２', '総合３', '左前１', '左前２', '左前３', '右前１',
                      '右前２', '右前３', '左後１', '左後２', '左後３', '右後１', '右後２', '右後３', 'パドックコメント', '脚元コメント', '馬具(その他)コメント', 'レース馬コメント', 'ハミ', 'バンテージ', '蹄鉄', '蹄状態',
                 'ソエ', '骨瘤', 'target_date']
        if filename[-3:] == "pkl":
            df = pd.read_pickle(self.jrdb_folder_path + type + '/' + filename)
        else:
